...
def draw_plot(list_in, title, filename):
    plt.plot(list_in)
    plt.title("Learning rate by Q-table size")
...
for i in range(5,41,5):
    print(f"i={i}")
    def train(os_size_0 = i, os_size_1 = i, pole_angular_velocity = 3.5, init_value_low = 0, init_value_high = 1, learning_rate = 0.1, discount=0.95, epsilon = 0.1, start_decay = 1, end_decay_at = 0.5):
    ...
    labels.append(f"OS = [{i},{i}]")
    draw_plot(train(), labels, f"os_{i}_{i}.png")
